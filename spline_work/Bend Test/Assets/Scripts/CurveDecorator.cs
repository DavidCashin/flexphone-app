...
public class CurveDecorator : MonoBehaviour
{
    ...
    private void Decorate()
    {
        ...
        for (int p = 0, f = 0; f < frequency; f++)
        {
            for (int i = 0; i < items.Length; i++, p++)
            {
                ...
                item.name = curve.name + "Cube" + cubeNumber;
                cubeNumber++;
    ...
    private void Redecorate()
    {
        ...
        for (int p = 0, f = 0; f < frequency; f++)
        {
            for (int i = 0; i < items.Length; i++, p++)
            {
                GameObject cube = GameObject.Find(curve.name + "Cube" + cubeNumber);
                cubeNumber++;
                Vector3 position = curve.GetPoint(p * stepSize);
                cube.transform.localPosition = position;
                if (lookForward)
                {
                    cube.transform.LookAt(position + curve.GetDirection(p * stepSize));
                }
            }
        }
    }
